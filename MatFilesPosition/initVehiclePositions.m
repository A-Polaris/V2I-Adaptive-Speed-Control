...
%        simValues.IVvalue=simParams.IVvalue;
    ...
    if ~simParams.Speed_Different
        ...
        simValues = DifSpeed(simParams,appParams,simValues);
    end
    for i=1:Nvehicles
    ...
    simValues.vReal = simValues.v * 3.6;
...
if appParams.nRSUs>0
    ...
    positionManagement.RSUid = idRSUs;
end
...
if simParams.adaSpeed
    positionManagement.vnumTH=(simValues.Xmax/simParams.NumofZones)/simParams.SafeDis+1;
    zonesRange=zeros(simParams.NumofZones,1);
    N=simParams.NumofZones;
    N_av=simValues.Xmax./N;
    for i=1:N
        zonesRange(i)=N_av.*i;
    end
    positionManagement.zonesRange=[zonesRange;zonesRange];
    [positionManagement,simValues,simParams]= zone_busy_ratio(positionManagement,simValues,simParams);

    positionManagement.InterSpace=zeros(1,simValues.maxID+1);
    positionManagement.currentColli=0;
end
if ~simParams.adaSpeed && simParams.NumColli
    [positionManagement,simValues,simParams]= VehicleOrder(positionManagement,simValues,simParams);
    positionManagement.InterSpace=zeros(1,simValues.maxID+1);
    positionManagement.currentColli=0;
end
