...
[simParams,varargin] = addNewParam(simParams,'Technology','NR-V2X','Choose if simulate "LTE-V2X", "NR-V2X" or "80211p"','string',fileCfg,varargin{1});
...
%% Dif 
[simParams,varargin] = addNewParam(simParams,'Speed_Different',false,'If using different speed','bool',fileCfg,varargin{1});
if simParams.Speed_Different
    [simParams,varargin] = addNewParam(simParams,'alldifSp',false,'If all V use different speed (true) or just 2 (false) speed','bool',fileCfg,varargin{1});
    [simParams,varargin] = addNewParam(simParams,'DifSpeed_Min',30,'Minimum Speed Limation (km/h)','double',fileCfg,varargin{1});
    [simParams,varargin] = addNewParam(simParams,'DifSpeed_Max',60,'Maximum Speed Limation (km/h)','double',fileCfg,varargin{1});
end
%% adaptive speed
[simParams,varargin] = addNewParam(simParams,'adaSpeed',false,'Use adaptive speed','bool',fileCfg,varargin{1});
[simParams,varargin] = addNewParam(simParams,'NumColli',false,'Calculting collision number','bool',fileCfg,varargin{1});
%% Safe dis
[simParams,varargin] = addNewParam(simParams,'SafeDis',25,'Safe distance (m)','double',fileCfg,varargin{1});         
if simParams.Speed_Different
    simParams.SafeDis = simParams.DifSpeed_Min ./ 3.6 * 3;  
end

%% zones
if simParams.adaSpeed
    [simParams,varargin] = addNewParam(simParams,'UseZone',true,'Scenario divided into zones','bool',fileCfg,varargin{1});
    [simParams,varargin] = addNewParam(simParams,'NumofZones',4,'Number of zones','integer',fileCfg,varargin{1});        
end
